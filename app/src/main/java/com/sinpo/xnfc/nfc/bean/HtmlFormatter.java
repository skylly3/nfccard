...
public final class HtmlFormatter {
	...
	private static boolean formatApplicationInfo(StringBuilder out, Application app) {
		...
		{
			SPEC.PROP prop = SPEC.PROP.HEAD;
		//	Bitmap bm = (Bitmap) app.getProperty(prop);
		//	if (bm != null)
		//	{
		...
		{
			SPEC.PROP prop = SPEC.PROP.SEX;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		
		{
			SPEC.PROP prop = SPEC.PROP.NATION;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		
		{
			SPEC.PROP prop = SPEC.PROP.BIRTHDAY;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		{
			SPEC.PROP prop = SPEC.PROP.ADDR;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		{
			SPEC.PROP prop = SPEC.PROP.IDNUM;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		{
			SPEC.PROP prop = SPEC.PROP.ISSUE;
			if (formatProperty(out, SPEC.TAG_LAB, prop, app.getStringProperty(prop)))
				newline(out);
		}
		{
