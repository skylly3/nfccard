...
public class Iso7816 {
	...
	public final static class StdTag {
		...
		//读取发卡方信息 by jl,2018.9.18
		public Response readBinary() throws IOException {
			final byte[] cmd = { (byte) 0x00, // CLA Class
					(byte) 0xB0, // INS Instruction
					(byte) 0x16, // P1 Parameter 1
					(byte) 0x00, // P2 Parameter 2
					(byte) 0x1E, // Le
			};
		...
		//擦除卡片 by jl,2018.9.18
		public Response earsecard() throws IOException {
			final byte[] cmd = { (byte) 0x80, // CLA Class
		...
		//add by jl,2018.9.12
		public Response verifypin(byte... pin) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(pin.length+5);
		...
		//add by jl,2018.9.18 ,外部认证
		public Response verifyout(byte... pin) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(pin.length+5);
			buff.put((byte) 0x00) // CLA Class
		...
		//add by jl,2018.9.17
		public Response unblockapp(byte... mac) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(mac.length+5);
			buff.put((byte) 0x84) // CLA Class
					.put((byte) 0x18) // INS Instruction
					.put((byte) 0x00) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) mac.length) // Lc
					.put(mac); //
			return new Response(transceive(buff.array()));
		}
		//add by jl,2018.9.14 乱写的，未验证 puk是8字节还是6字节？
		public Response unblockypin(byte... puk) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(puk.length+5);
			buff.put((byte) 0x00) // CLA Class
					.put((byte) 0x24) // INS Instruction
					.put((byte) 0x00) // P1 Parameter 1
					.put((byte) 0x00) // P2 Parameter 2
					.put((byte) puk.length) // Lc
					.put(puk); //
			return new Response(transceive(buff.array()));
		}
		//add by jl,2018.9.17 取随机数
		public Response getrand(int iLen) throws IOException {
		...
		//add by jl,2018.9.12  初始化XXX交易
		public Response precharge(byte[] amount, byte[] posid) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(amount.length + posid.length + 6);
			buff.put((byte) 0x80) // CLA Class
					.put((byte)0x50) // INS Instruction
					.put((byte)0x00).put((byte)0x02)   //0002-P1P2-电子钱包
					.put((byte)0x0B).put((byte)0x01)
					.put(amount) // P2 Parameter 2
					.put(posid); // Lc

			return new Response(transceive(buff.array()));
		}
		//add by jl,2018.9.12  CREDIT FOR LOAD 圈存
		public Response charge(byte[] datetime, byte[] mac2) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(datetime.length + mac2.length + 5);
			buff.put((byte) 0x80) // CLA Class
					.put((byte) 0x52) // INS Instruction
					.put((byte)0).put((byte)0).put((byte)11)
					.put(datetime) // P2 Parameter 2
					.put(mac2); // Lc

			return new Response(transceive(buff.array()));
		}
		//计算mac 2018.9.18
		public Response calcmac(byte... pdol) throws IOException {
			ByteBuffer buff = ByteBuffer.allocate(pdol.length + 6);
			buff.put((byte) 0x80) // CLA Class
					.put((byte) 0xFA) // INS Instruction
