...
public final class NfcManager {
	...
	static {
		try {
			TECHLISTS = new String[][] { 
				{ IsoDep.class.getName() },   //中国人民银行卡/城市一卡通
				{ NfcA.class.getName() },     //MifareClassic或IsoDep
				{ NfcB.class.getName() },     //二代证		
				{ NfcF.class.getName() }, 	  //索尼的Felica卡
				{ NfcV.class.getName() },     //VicinityCard卡
				{ Ndef.class.getName() },     //Android分享文件
				{ NdefFormatable.class.getName() },   	//Android分享文件
				{ MifareClassic.class.getName() },   		  //M1卡			
	...
	public boolean readCard(Context ctx, Intent intent, ReaderListener listener) {
		final Tag tag = (Tag) intent.getParcelableExtra(EXTRA_TAG);
		if (tag != null) {
			ReaderManager.readCard(ctx, intent, tag, listener);
			return true;
		}
		return false;
	}
